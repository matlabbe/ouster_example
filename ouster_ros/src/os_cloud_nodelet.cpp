...
class OusterCloud : public nodelet::Nodelet {
   private:
    virtual void onInit() override {
        ...
        filter_param_a_ = pnh.param("filter_param_a", 0.0);
		filter_param_b_ = pnh.param("filter_param_b", 0.0);
		filter_param_c_ = pnh.param("filter_param_c", 0.0);
    ...
    void convert_scan_to_pointcloud_publish(std::chrono::nanoseconds scan_ts,
        ...
        for (int i = 0; i < n_returns; ++i) {
            if(fixed_frame_.empty()) {
                scan_to_cloud(xyz_lut, scan_ts, ls, cloud, i, filter_param_a_, filter_param_b_, filter_param_c_);
            } else {
                scan_to_cloud(xyz_lut, scan_ts, ls, cloud, i, filter_param_a_, filter_param_b_, filter_param_c_, listener_, fixed_frame_, sensor_frame, wait_for_transform_);
            }
   ...
   private:
    ...
    double filter_param_a_;
    double filter_param_b_;
    double filter_param_c_;
};
}  // namespace nodelets_os
